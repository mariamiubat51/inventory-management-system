    ...
    <form action="{{ route('purchases.store') }}" method="POST">
            ...
            <tbody>
                @if(old('product_id'))
                    ...
                    <tr>
                        <td>
                            <select name="product_id[]" class="form-control product-select" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $product->id == $oldProductId ? 'selected' : '' }}>
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <td>
                            <input type="number" name="quantity[]" class="form-control qty" min="1" required value="{{ old('quantity')[$index] ?? 1 }}">
                        </td>
                        <td>
                            <input type="number" name="buying_price[]" class="form-control price" step="0.01" min="0" required value="{{ old('buying_price')[$index] ?? '' }}">
                        </td>
                ...
                @else
                    <tr>
                        ...
                        <td><input type="number" name="buying_price[]" class="form-control price" step="0.01" min="0" required></td>
                        <td><input type="text" class="form-control subtotal" readonly></td>
                        <td><button type="button" class="btn btn-danger remove-row">❌</button></td>
                    </tr>
                @endif
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <input type="number" name="paid_amount" class="form-control" step="0.01" min="0" required value="{{ old('paid_amount') }}">
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="due_amount" class="form-control" step="0.01" min="0" readonly value="{{ old('due_amount', '0') }}">
            </div>
            <div class="col-md-4">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateSubtotals() {
        ...
        if (!paidInput.dataset.manual) {
            ...
            // If paid amount manually changed, recalc due
